...
import {loginUser , change_log_stat} from './redux/actions.js'
...
class Login extends React.Component{
    ...
    getHandler = key => value =>{
        ...
        if(key==="email" && value.length>0 && !emailRegx.exec(value.toString().toLowerCase())) this.setState({email_val : true})
        else if(key === "email" && value.length===0) this.setState({email_val : false})
        if(key === "password" && value.length>0 && !passwordRegx.exec(value.toString())) this.setState({password_val : true})
        else if(key === "password" && value.length===0) this.setState({password_val : false})
    }
    ...
    validate = () => {
        ...
        if(emailRegx.exec(this.state.email.toString().toLowerCase())) this.setState({email_val : false})
        //else this.setState({email_val : true})
        if(passwordRegx.exec(this.state.password.toString())) this.setState({password_val : false})
    ...
    render(){
        return(
            <View style={styles.full_container}>
                 <View>
                    <Header title="Login" back_button={false}/>
                </View>
                
                <KeyboardAvoidingView style={styles.center_data}>
                    <View style={styles.container}>
                        <TextInput label="Email" style={styles.input} theme={themes.username_theme} keyboardType="default" mode="outlined" value={this.state.email.trim()} onChangeText={this.getHandler('email')}/>
    ...
    static getDerivedStateFromProps(nextProps, prevState){
        if(nextProps.user.user_info){
            console.log("Now I will navigate to new page")
...
export default connect(msp , {loginUser , change_log_stat})(Login)
